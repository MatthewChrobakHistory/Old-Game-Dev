    ...
    Do While ServerOnline
        ...
        If Tick > tmr25 Then
            For i = 1 To Player_HighIndex
                ...
                End If
            Next
            frmServer.lblFPS.Caption = "FPS: " & Format$(GameFPS, "#,###,###,###")
                                        ...
                                        If DistanceX <= n And DistanceY <= n Then
                                            If Npc(npcNum).Behaviour = NPC_BEHAVIOUR_ATTACKONSIGHT Or GetPlayerPK(i) = YES Then
                                                If Len(Trim$(Npc(npcNum).AttackSay)) > 0 Then
                                                    Call PlayerMsg(i, CheckGrammar(Trim$(Npc(npcNum).Name), 1) & " says: " & Trim$(Npc(npcNum).AttackSay), SayColor)
                                                End If
