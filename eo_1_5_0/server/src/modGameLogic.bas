...
Dim expShare As Long, leftOver As Long, i As Long, tmpIndex As Long
    ...
    For i = 1 To MAX_PARTY_MEMBERS
        ...
        If tmpIndex > 0 Then
            ...
            If IsConnected(tmpIndex) And IsPlaying(tmpIndex) Then
                ' give the exp
                Call SetPlayerExp(tmpIndex, GetPlayerExp(tmpIndex) + expShare)
                SendEXP tmpIndex
                SendActionMsg GetPlayerMap(tmpIndex), "+" & expShare & " EXP", White, 1, (GetPlayerX(tmpIndex) * 32), (GetPlayerY(tmpIndex) * 32)
                ' check if we've leveled
                CheckPlayerLevelUp tmpIndex
            End If
        End If
    Next
    ...
    ' give the exp
    Call SetPlayerExp(tmpIndex, GetPlayerExp(tmpIndex) + leftOver)
    SendEXP tmpIndex
    SendActionMsg GetPlayerMap(tmpIndex), "+" & leftOver & " EXP", White, 1, (GetPlayerX(tmpIndex) * 32), (GetPlayerY(tmpIndex) * 32)
    ' check if we've leveled
    CheckPlayerLevelUp tmpIndex
End Sub
