...
Private Sub HandleLogin(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    ItemReward = frmServer.scrlItemReward.Value
    ...
    If Not IsPlaying(index) Then
        If Not IsLoggedIn(index) Then
            ...
            If frmServer.chkAllowDaily.Value = 1 Then
            ...
            If Player(index).DailyValue <> frmServer.lblDailyValue.Caption Then
                ...
                End If
                If FindOpenInvSlot(index, 1) <> 0 Then
                    GiveInvItem index, ItemReward, 0
...
Call PlayerMsg(index, Trim$(Name) & " is level " & GetPlayerLevel(i), White)
Call PlayerMsg(index, "Strength: " & GetPlayerStat(i, Strength) & " // " & "Endurance: " & GetPlayerStat(i, Endurance) & "Intelligence: " & GetPlayerStat(i, intelligence) & " // Agility: " & GetPlayerStat(i, Agility) & " // Willpower " & GetPlayerStat(i, willpower), White)
Call PlayerMsg(index, vbNewLine, White)
Call PlayerMsg(index, "Woodcutting: " & GetPlayerWoodcuttingXP(i) & " // Fishing: " & GetPlayerFishingXP(i) & " // Mining: " & GetPlayerMiningXP(i) & " // Potion Brewing: " & GetPlayerPotionBrewingXP(i) & " // Smithing: " & GetPlayerSmithingXP(i) & " // Cooking: " & GetPlayerCookingXP(i) & " // Fletching: " & GetPlayerFletchingXP(i) & " // Crafting: " & GetPlayerCraftingXP(i), White)
...
With Shop(shopNum).TradeItem(shopslot)
         ...
         If Shop(shopNum).ShopType > 0 Then
                 ...
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).CrXP > Player(index).CraftingXP Then
                         PlayerMsg index, "You need " & Item(Shop(shopNum).TradeItem(.Item).Item).CoXP - Player(index).CraftingXP & " more crafting experience to make this item.", BrightRed
                 Exit Sub
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).PBXP > Player(index).PotionBrewingXP Then
                        PlayerMsg index, "You need " & Item(Shop(shopNum).TradeItem(shopslot).Item).PBXP - Player(index).PotionBrewingXP & " more potion brewing experience to make this item.", BrightRed
                    Exit Sub
                 End If
                 ...
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).PBRew > 0 Then
                        Player(index).PotionBrewingXP = Player(index).PotionBrewingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).PBRew
                 End If
                 SendPlayerData (index)
         End If
...
Sub HandleSellItem(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    ' item has cost?
    If price <= 0 Then
        PlayerMsg index, "That item can't be sold.", BrightRed
        Exit Sub
    End If
