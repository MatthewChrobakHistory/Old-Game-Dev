...
Sub ServerLoop()
    ...
    Dim tmr25 As Long, tmr500 As Long, tmr1000 As Long, tmr15000 As Long
    ...
    Do While ServerOnline
        ...
        'XPOISONX
        If Tick > tmr15000 Then
            For i = 1 To Player_HighIndex
                If Player(i).PoisonTick > 0 Then
                    If GetPlayerVital(i, HP) - Player(i).PoisonDamage <= 0 Then
                        KillPlayer (i)
                    Else
                        Call SetPlayerVital(i, Vitals.HP, Vitals.HP - Player(i).PoisonDamage)
                        Player(i).PoisonTick = Player(i).PoisonTick - 1
                        SendActionMsg GetPlayerMap(i), "-" & Player(i).PoisonDamage, BrightGreen, 1, GetPlayerX(i) * 32, GetPlayerY(i) * 32
                        Call SendPlayerData(i)
                    End If
                End If
            Next
            tmr15000 = GetTickCount + 15000
        End If
