    ...
    ' Party Msg
        ...
        If Left$(ChatText, 1) = "'" Then
            ...
            If Player(MyIndex).Access < 1 Then
                Call AddText("You need to be a higher access level to access this feature!", BrightRed)
                Exit Sub
            End If
...
' Player message
        If Left$(ChatText, 3) = "/t " Then
            ...
            Else
                Call AddText("Usage: /t playername (message)", AlertColor)
            End If
        ...
        If Left$(MyText, 1) = "/" Then
            ...
            Select Case Command(0)
                Case "/help"
                    ...
                    Call AddText("/t namehere msghere = Player Message", HelpColor)
                    Call AddText("/p msghere = Party Message", HelpColor)
                    Call AddText("Available Commands: /info, /who, /fps, /fpslock", HelpColor)

                Case "/dice"
                
                    If GetPlayerAccess(MyIndex) < ADMIN_MAPPER Then
                        Call AddText("You must be a member to dice!", BrightRed)
                    End If
